/**
 * JS administrator related functionality.
 *
...
jQuery( document ).ready(
	function() {
		var wrapFaces = function() {
			...
			if ( faces ) {
				...
				for ( var i in faces ) {
					var face    = faces[ i ];
					var size    = face.width * face.height;
					var ratioX  = 100 * image.width() / container.width();
					var ratioY  = 100 * image.height() / container.height();
					var wrapper = jQuery( '<div/>' )
						.addClass( 'face-detection-face' )
						.css(
							{
								left   : Math.round( ratioX * face.x / face_detection.original_width ) + '%',
								top    : Math.round( ratioY * face.y / face_detection.original_height ) + '%',
								width  : Math.round( ratioX * face.width / face_detection.original_width ) + '%',
								height : Math.round( ratioY * face.height / face_detection.original_height ) + '%',
							}
						);
		...
		if ( 'undefined' !== typeof window.face_detection ) {
			...
			jQuery( '#face-detection-regenerate' ).click(
				function( e ) {
					e.preventDefault();
			...
			jQuery( '#face-detection-reset' ).click(
				function( e ) {
					e.preventDefault();
